...
class MultiSpaceL1Loss(nn.Module):
    def __init__(self, w_rgb=1.0, w_hsv=1.0, w_lab=1.0):
        super().__init__()
        self.w_rgb = w_rgb
        self.w_hsv = w_hsv
        self.w_lab = w_lab
    ...
    def forward(self, pred, target, return_log=False):
        if kornia is None:
            raise ImportError("kornia is required for MultiSpaceL1Loss.")
        ...
        pred_hsv = kornia.color.rgb_to_hsv(pred_clamped)
        tgt_hsv = kornia.color.rgb_to_hsv(target_clamped)
        loss_hsv = self.l1(pred_hsv, tgt_hsv)

        pred_lab = kornia.color.rgb_to_lab(pred_clamped)
        tgt_lab = kornia.color.rgb_to_lab(target_clamped)
        loss_lab = self.l1(pred_lab, tgt_lab)
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        criterion = MultiSpaceL1Loss(w_rgb=1.0, w_hsv=0.8, w_lab=1.2)
        target = torch.rand_like(enhance).clamp(0, 1)
        loss, log = criterion(enhance, target, return_log=True)
        print("Multi-space loss:", float(loss))
        print("Loss log:", {k: float(v) for k, v in log.items()})
